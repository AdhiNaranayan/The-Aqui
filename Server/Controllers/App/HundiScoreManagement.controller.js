     ...
     Promise.all([
        ...
        if (CustomerDetails !== null) { 
           ...
           if (ReceivingData.CustomerCategory === 'Seller') {
              if (CustomerDetails.CustomerType === 'Owner') {
                ...
                if (OwnerBusinessDetails.length > 0) {
                 ...
                 ChartAmountsInvoiceQuery = { Seller: ReceivingData.CustomerId, InvoiceStatus: "Accept", ActiveStatus: true, IfDeleted: false };
                ...
                if (CustomerDetails.BusinessAndBranches.length > 0) {
                 ...
                 ChartAmountsInvoiceQuery = { Business: { $in: UserBusinessesArr }, InvoiceStatus: "Accept", ActiveStatus: true, IfDeleted: false };
              ...
              if (CustomerDetails.CustomerType === 'Owner') {
                ...
                if (OwnerBusinessDetails.length > 0) {
                 ...
                 ChartAmountsInvoiceQuery = { Buyer: ReceivingData.CustomerId, InvoiceStatus: "Accept", ActiveStatus: true, IfDeleted: false };
                ...
                if (CustomerDetails.BusinessAndBranches.length > 0) {
                 ...
                 ChartAmountsInvoiceQuery = {  BuyerBusiness: { $in: UserBusinessesArr }, InvoiceStatus: "Accept", ActiveStatus: true, IfDeleted: false };
           ...
           Promise.all([
              ...
              InvoiceManagement.InvoiceSchema.find(ChartAmountsInvoiceQuery, {}, {}).exec(),
              ...
              var ChartInvoiceAmounts = JSON.parse(JSON.stringify(Response[14]));

                // console.log(ChartInvoiceAmounts,'ChartInvoiceAmountsChartInvoiceAmounts');
              ...
              if (ChartInvoiceAmounts.length !== 0) {

                 var OverDueAmount = 0;
                 var DueTodayAmount = 0;
                 var UpComingAmount = 0;
                 var UpcomingChartAmounts = 0;
                ...
                ChartInvoiceAmounts.map(Obj => {
                    // console.log(Obj.ChartAmount,'43236',Obj.InvoiceNumber);
                  ...
                  // var mCurrentDateDate = moment(TodayDate);
                      ...
                      if (Obj.ChartAmount) {
                        if (mInvoiceDate.isBefore(TodayDate, 'day')) {
                          OverDueAmount = parseFloat(OverDueAmount) + parseFloat(Obj.ChartAmount);
                      } else if (mInvoiceDate.isSame(TodayDate, 'day')) {
                          DueTodayAmount = parseFloat(DueTodayAmount) + parseFloat(Obj.ChartAmount);
                      } else {
                            if (Obj.ChartAmount != 0) {
                                    
                              UpcomingChartAmounts += Obj.ChartAmount ;
                              // console.log(UpcomingChartAmounts,'234234');
                              UpComingAmount = parseFloat(UpcomingChartAmounts);

                            } else if (Obj.ChartAmount === 0) {
                              UpComingAmount = 0;
     ...
     CustomerManagement.CustomerSchema.findOne({ _id: ReceivingData.CustomerId, ActiveStatus: true, IfDeleted: false }, {}, {}).exec(function (err, result) {
        if (err) {
           ...
           if (result !== null) {
              ...
              var InviteQuery = {};
              var InvoicePendingQuery = {};
              var ChartInvoiceQuery = {};
              var BranchArr = [];
              var BusinessArr = [];
              if (result.CustomerType === 'Owner') {
                 if (ReceivingData.CustomerCategory === 'Seller') {
                    InvoiceQuery = { PaidORUnpaid: "Unpaid", Seller: ReceivingData.CustomerId, InvoiceStatus: "Accept", ActiveStatus: true, IfDeleted: false };
                    ChartInvoiceQuery = { Seller: ReceivingData.CustomerId,InvoiceStatus: "Accept", ActiveStatus: true, IfDeleted: false };
             ...
             InvoicePendingQuery = { Seller: ReceivingData.CustomerId, InvoiceStatus: "Pending", ActiveStatus: true, IfDeleted: false };
                    ...
                    ChartInvoiceQuery = { Buyer: ReceivingData.CustomerId, InvoiceStatus: "Accept", ActiveStatus: true, IfDeleted: false };
            ...
            if (ReceivingData.CustomerCategory === 'Seller') {
              ...
              ChartInvoiceQuery = { Business: { $in: BusinessArr }, InvoiceStatus: "Accept", ActiveStatus: true, IfDeleted: false };
              ...
              ChartInvoiceQuery = { BuyerBusiness: { $in: BusinessArr }, InvoiceStatus: "Accept", ActiveStatus: true, IfDeleted: false };
          ...
          if ((ReceivingData.CustomerCategory === 'Buyer' && ReceivingData.FilterQuery.Business !== '') || (ReceivingData.CustomerCategory === 'Seller' && ReceivingData.FilterQuery.BuyerBusiness !== '')) {
            // returnMode = 'Business';
              ...
              Promise.all([
                 ...
                 .populate({ path: "Business", select: ['FirstName','LastName','BusinessUnique_id'] })
                 .populate({ path: "BuyerBusiness", select: ['FirstName','LastName','BusinessUnique_id'] })
                 .exec(),
                  InvoiceManagement.InvoiceSchema.find(InvoicePendingQuery, {}, {}).exec(),
                  InvoiceManagement.InvoiceSchema.find(ChartInvoiceQuery, {}, {}).exec(),
              ]).then(Response => {
                 var InvoiceDetails = JSON.parse(JSON.stringify(Response[0]));
                 var TemporaryActiveDetails = JSON.parse(JSON.stringify(Response[1]));
                 var InviteDetails = JSON.parse(JSON.stringify(Response[2]));
                 var PendingInvoice = JSON.parse(JSON.stringify(Response[3]));
                 var ChartInvoiceAmounts = JSON.parse(JSON.stringify(Response[4]));
                       ...
                       { $match: { _id: { $in: CustomerId } } },
                       {
                          $lookup: {
                             ...
                             pipeline: [
                                ...
                                { $project: {  "FirstName": 1,"LastName": 1,"BusinessUnique_id":1} }
                             ],
                             as: 'Businesses'
                          }
                       },
                          ...
                          $group: {
                             ...
                             Businesses: {
                                $push: {
                                   ...
                                   BusinessUnique_id :'$Businesses.BusinessUnique_id'
                                }
                             },
                          }
                       }
              ...
              var returnList = ConnectedCustomer.map(ObjCustomer => {
                ...
                if (ChartInvoiceAmounts.length > 0) {
                  const InvoiceDetailsArray = InvoiceDetails.filter(obj1 => obj1.Business === objBranch._id || obj1.BuyerBusiness === objBranch._id);
                  const ChartDetailsArray = ChartInvoiceAmounts.filter(obj1 => obj1.Business === objBranch._id || obj1.BuyerBusiness === objBranch._id);
                  
                  if (ChartDetailsArray.length > 0) {
                      // InvoiceDetailsArray.forEach(Obj => {
                      
                      // });
                      ChartDetailsArray.map(chart=>{
                        var InvoiceDate = moment(chart.InvoiceDueDate).startOf('day');
            
                        if (InvoiceDate.isBefore(Today, 'day')) {
                            newBranchObj.OverDueAmount += parseFloat(chart.ChartAmount);
                        } else if (InvoiceDate.isSame(Today, 'day')) {
                            newBranchObj.DueTodayAmount += parseFloat(chart.ChartAmount);
                        } else if (InvoiceDate.isAfter(Today, 'day')) {
                            newBranchObj.UpComingAmount += parseFloat(chart.ChartAmount);
                        }
                      })
...
exports.ConnectedCustomerWithFilter = function (req, res) {
  var ReceivingData = req.body;
     ...
     CustomerManagement.CustomerSchema.findOne({ _id: ReceivingData.CustomerId, ActiveStatus: true, IfDeleted: false }, {}, {}).exec(function (err, result) {
        if (err) {
           ...
           if (result !== null) {
            ...
            if (ReceivingData.CustomerCategory === 'Seller') {
              InviteQuery = { Seller: result.Owner, Invite_Status: "Accept" };
            } else if (ReceivingData.CustomerCategory === 'Buyer') {
              InviteQuery = { Buyer: result.Owner, Invite_Status: "Accept" };
            }
           }
           ...
           // find all customers from ivite table
           InviteManagement.InviteManagementSchema.find(InviteQuery, {}, {})
              .populate({ path: "Business", select: ['FirstName','LastName','BusinessUnique_id'] })
              .populate({ path: "BuyerBusiness", select: ['FirstName','LastName','BusinessUnique_id'] })

              .exec(function (err, InviteDetails) {
                  if (err) {
                      ErrorHandling.ErrorLogCreation(req, 'Invite Details Getting Error', 'HundiScore.Controller -> Invite Details Finding Error', JSON.stringify(err));
                      res.status(417).send({ Status: false, Message: "Some errors predicted, We are unable to get Invite Details!.", Error: err });
                  } else {
                    if (InviteDetails.length > 0) {
                      var customers = [];
                      //console.log('InviteDetails', InviteDetails);

                      const Promises = InviteDetails.map(async Obj => {
                        if (result.CustomerType === 'User') {
                          const userBusinessList = result.BusinessAndBranches
                          if (ReceivingData.CustomerCategory === 'Seller') {
                            if (userBusinessList.length > 0) {
                              const userBusiness = userBusinessList.find(obj => String(obj.Business) === String(Obj.Business._id))
                              if (!userBusiness) {
                                return
                              }
                          }
                          ...
                          if (userBusinessList.length > 0) {
                            // console.log('Obj.BuyerBusiness._id', Obj.BuyerBusiness._id);
                            // console.log('userBusinessList', userBusinessList[0].Business);
                            const userBusiness = userBusinessList.find(obj => String(obj.Business) === String(Obj.BuyerBusiness._id))
                            if (!userBusiness) {
                              return
                            }
                          }
                      ...
                      const individualCustomerInfo = {
                        _id: Obj._id,
                        Name: '',
                        Mobile: '',
                        FirstName: '',
                        LastName: '',
                        TotalCreditLimit: Obj.BuyerCreditLimit,
                        AvailableCreditLimit: Obj.AvailableLimit,
                        TotalTemporaryLimit: 0,
                        AvailableTemporaryLimit: 0,
                        OverDueAmount: 0,
                        UpComingAmount: 0,
                        ...
                        inviteData: {
                            InviteId: Obj._id,
                            BuyerPaymentCycle: Obj.BuyerPaymentCycle,
                      ...
                      if (ReceivingData.CustomerCategory === 'Seller') {
                        individualCustomerInfo.Name = Obj.Business.FirstName + ' ' + Obj.Business.LastName;
                        individualCustomerInfo.FirstName = ReceivingData.FilterQuery.Business ? Obj.BuyerBusiness.FirstName : Obj.Business.FirstName;
                        individualCustomerInfo.LastName = ReceivingData.FilterQuery.Business ? Obj.BuyerBusiness.LastName : Obj.Business.LastName;
                        individualCustomerInfo.BuyerID = Obj.Buyer;
                        individualCustomerInfo.SellerID = Obj.Seller;
                        // find business details
                        const businessDetails = await BusinessAndBranchManagement.BusinessSchema.findOne({ _id: Obj.BuyerBusiness._id }, { FirstName: 1, LastName: 1, BusinessCreditLimit: 1, AvailableCreditLimit: 1 }, {}).exec();
                        if (businessDetails !== null) {
                          // individualCustomerInfo.TotalCreditLimit = businessDetails.BusinessCreditLimit;
                          // individualCustomerInfo.AvailableCreditLimit = businessDetails.AvailableCreditLimit;

                          const InvoiceDetails = await InvoiceManagement.InvoiceSchema.find({  Business: Obj.Business, StatusType: 'uncleared' }, {}).exec();
                          
                          if (InvoiceDetails.length > 0) {
                            InvoiceDetails.map(ObjIn => {
                              var InvoiceDate = new Date(ObjIn.InvoiceDueDate);
                              var Today = new Date(new Date().setHours(0, 0, 0, 0));
                              if (InvoiceDate.valueOf() < Today.valueOf()) {
                               
                                individualCustomerInfo.OverDueAmount = individualCustomerInfo.OverDueAmount + parseFloat(ObjIn.InvoiceAmount - ObjIn.PaidAmount);
                              }
                              if (InvoiceDate.valueOf() === Today.valueOf()) {
                                individualCustomerInfo.DueTodayAmount = individualCustomerInfo.DueTodayAmount + parseFloat(ObjIn.InvoiceAmount - ObjIn.PaidAmount);
                              }
                              if (InvoiceDate.valueOf() > Today.valueOf()) {
                                individualCustomerInfo.UpComingAmount = individualCustomerInfo.UpComingAmount + parseFloat(ObjIn.InvoiceAmount - ObjIn.PaidAmount);
                              }
                            });
                            individualCustomerInfo.OverDueAmount = parseFloat(individualCustomerInfo.OverDueAmount.toFixed(2));
                            individualCustomerInfo.DueTodayAmount = parseFloat(individualCustomerInfo.DueTodayAmount.toFixed(2));
                            individualCustomerInfo.UpComingAmount = parseFloat(individualCustomerInfo.UpComingAmount.toFixed(2));
                          }
                        }
                        // finc customer phone number and name
                        const customerDetails = await CustomerManagement.CustomerSchema.findOne({ _id: Obj.Buyer }, { ContactName: 1, Mobile: 1 }, {}).exec();
                        if (customerDetails !== null) {
                          individualCustomerInfo.Name = customerDetails.ContactName;
                          individualCustomerInfo.Mobile = customerDetails.Mobile;
                        }

                      } else if (ReceivingData.CustomerCategory === 'Buyer') {
                        individualCustomerInfo.Name = Obj.BuyerBusiness.FirstName + ' ' + Obj.BuyerBusiness.LastName;
                        individualCustomerInfo.FirstName = ReceivingData.FilterQuery.BuyerBusiness ? Obj.Business.FirstName : Obj.BuyerBusiness.FirstName;
                        individualCustomerInfo.LastName = ReceivingData.FilterQuery.BuyerBusiness ? Obj.Business.LastName : Obj.BuyerBusiness.LastName;
                        individualCustomerInfo.BuyerID = Obj.Buyer;
                        individualCustomerInfo.SellerID = Obj.Seller;
                        // find business details
                        const businessDetails = await BusinessAndBranchManagement.BusinessSchema.findOne({ _id: Obj.Business._id }, { FirstName: 1, LastName: 1, BusinessCreditLimit: 1, AvailableCreditLimit: 1 }, {}).exec();

                        if (businessDetails !== null) {
                          // individualCustomerInfo.TotalCreditLimit = businessDetails.BusinessCreditLimit;
                          // individualCustomerInfo.AvailableCreditLimit = businessDetails.AvailableCreditLimit;  
                        
                          const InvoiceDetails = await InvoiceManagement.InvoiceSchema.find({ Buyer:Obj.Buyer,BuyerBusiness: Obj.BuyerBusiness, StatusType: 'uncleared' }, {}).exec();
                        ...
                        // finc customer phone number and name
                        const customerDetails = await CustomerManagement.CustomerSchema.findOne({ _id: Obj.Seller }, { ContactName: 1, Mobile: 1 }, {}).exec();
                        if (customerDetails !== null) {
                          individualCustomerInfo.Name = customerDetails.ContactName;
                          individualCustomerInfo.Mobile = customerDetails.Mobile;
                        }
                    ...
                    Promise.all(Promises).then(Response => {
                      customers = Response;
                      res.status(200).send({ Status: true, Message: "Connected Customer!.", Response: customers });
                    }).catch(Error => {
                      ErrorHandling.ErrorLogCreation(req, 'Business Details Getting Error', 'HundiScore.Controller -> Business Details Finding Error', JSON.stringify(Error));
                      res.status(417).send({ Status: false, Message: "Some errors predicted, We are unable to get Business Details!.", Error: Error });
                    });
                    
                      
                    } else {
                      res.status(200).send({Status: true, Message: "Sorry You dont have any connected Customers!", Response: []});
                    }
                  }
...
exports.BuyerStatusType_Update = async (req, res) => {
  let ReceivingData = req.body;
  if (!ReceivingData.InviteId || ReceivingData.InviteId === '') {
    return res.status(400).send({ Status: false, Message: "InviteId cannot be empty"});
  } if (!ReceivingData.StatusType || ReceivingData.StatusType === '') {
    return res.status(400).send({ Status: false, Message: "Invalid StatusType"});
  }
   
    InviteManagement.InviteManagementSchema.findOne({ "_id": ReceivingData.InviteId })
     .populate({ path: 'Customer', select: ['TotalCreditBalance', 'AvailableCreditBalance'] })
     .exec(async (error, inviteData) => {
        if (error) {
          res.status(417).send({ Status: false, Message: "Something went wrong!", Error: error });
        } else if (!inviteData) {
          res.status(404).send({ Status: false, Message: "Invite not found" });
        } else {
          try {
            let data;
            // console.log(`InviteData`, inviteData);
            let invoiceData = await InvoiceManagement.InvoiceSchema.find({Buyer: mongoose.Types.ObjectId(inviteData.Buyer), BuyerBusiness: mongoose.Types.ObjectId(inviteData.BuyerBusiness), Business: mongoose.Types.ObjectId(inviteData.Business)}) 
            let invoiceStatus = invoiceData.map((item) => { return { status : item.InvoiceStatus}})
            if(!invoiceData) {
              return res.status(401).send({ Status: false, Message: "Invoice not found for business"})
            }
          if(ReceivingData.StatusType == 'Inactive'){
            invoiceData.map(async (item) => { 
              const status = item.StatusType;
              // console.log(`status`, status);
              if((status == 'uncleared') && (inviteData.BuyerCreditLimit == inviteData.AvailableLimit)){
                data = await InviteManagement.InviteManagementSchema.updateOne({ "_id": ReceivingData.InviteId }, { $set: { StatusType: ReceivingData.StatusType  } })
                return res.status(200).send({ Status: true, response:`Buyer Status updated to Inactive successfully`})
              } else {
                return res.status(401).send({ Status: false, message: `credit limit is not yet closed`})
              }
             })
            // if((invoiceData.InvoiceStatus == "closed") &&(inviteData.BuyerCreditLimit == inviteData.AvailableLimit)){
            //   data = await InviteManagement.InviteManagementSchema.updateOne({ "_id": ReceivingData.InviteId }, { $set: { StatusType: ReceivingData.StatusType  } })
            //   return res.status(200).send({ Status: true, response:`Buyer Status updated to Inactive successfully`})
            // } else {
            //   return res.status(401).send({ Status: false, message: `credit limit is not yet closed`})
          ...
          if(ReceivingData.StatusType == 'active') {
            data = await InviteManagement.InviteManagementSchema.updateOne({ "_id": ReceivingData.InviteId }, { $set: { StatusType: ReceivingData.StatusType  } })
            return res.status(200).send({ Status: true, response: `Buyer Status updated to active successfully`})
          }
          } catch (error) {
...
exports.SellerStatusType_Update = async (req, res) => {
  let ReceivingData = req.body;
  if (!ReceivingData.InviteId || ReceivingData.InviteId === '') {
    return res.status(400).send({ Status: false, Message: "InviteId cannot be empty"});
  } if (!ReceivingData.StatusType || ReceivingData.StatusType === '') {
