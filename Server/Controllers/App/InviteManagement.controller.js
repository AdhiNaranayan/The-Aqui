  ...
  if (!ReceivingData.InviteId || ReceivingData.InviteId === '') {
    ...
    Promise.all([
      ...
      InviteManagement.InviteManagementSchema.find({
        Seller: ReceivingData.CustomerId, InviteProcess:"Pending",
        // $or: [{ Invite_Status: 'Pending_Approval' }, { Invite_Status: 'Pending' }, { Invite_Status: 'Accept' }],
      ...
      var AlreadyInvited = Response[4];
      var ExistInviteDetails = [];
      // console.log(AlreadyInvited);
      if (AlreadyInvited !== null) {
        await Promise.all(AlreadyInvited.map(async obj => {
          
          var BuyerBusinessId = mongoose.Types.ObjectId(obj.BuyerBusiness);
          var BusinessId = mongoose.Types.ObjectId(ReceivingData.Business);
          // console.log(BuyerBusinessId, typeof (BuyerBusinessId), 'BuyerBusinessId')
          //   console.log(BusinessId,'BusinessId',typeof(BusinessId));
          const details = await InviteManagement.InviteManagementSchema.find({
            BuyerBusiness: BuyerBusinessId,
            Business: BusinessId
          });
            // Check if details already exist in ExistInviteDetails to avoid duplication
            if (details.length > 0) {
              const exists = ExistInviteDetails.some(existingDetails => existingDetails._id.toString() === details[0]._id.toString());
              if (!exists) {
                ExistInviteDetails.push(details[0]); // Push the first element of details array
              }
            }
        }));
      }
    
      if (ExistInviteDetails !== null && ExistInviteDetails.length > 0) {
        res.status(400).send({ Status: false, Message: "Sorry! Already You have Invited to the Business "});
      } else if (ExistInviteDetails.length === 0) {
        if (CustomerDetails !== null && InviteDetails !== null) {
          ReceivingData.BuyerCreditLimit = parseFloat(ReceivingData.BuyerCreditLimit);
          if (ReceivingData.BuyerCreditLimit > 0) {
            ReceivingData.BuyerCreditLimit = ReceivingData.BuyerCreditLimit.toFixed(2);
...
exports.SellerUpdateToBuyerCreditLimit = async function (req, res) {
  var ReceivingData = req.body;
  ...
  if (!ReceivingData.InviteId || ReceivingData.InviteId === '') {
    ...
    try {
      const [
        ...
        CustomersManagement.CustomerSchema.findOne({ _id: ReceivingData.CustomerId, ActiveStatus: true, IfDeleted: false }),
        InviteManagement.InviteManagementSchema.findOne({ _id: ReceivingData.InviteId, ActiveStatus: true, IfDeleted: false })
          .populate({ path: "Business", select: ["FirstName", "LastName"] })
          .populate({ path: "BuyerBusiness", select: ["FirstName", "LastName"] })
          .populate({ path: "Buyer", select: ["Firebase_Token", "Device_Type", "Mobile"] }),
        CustomersManagement.CustomerSchema.find({ "BusinessAndBranches.Business": { $in: SellerBusiness }, CustomerType: 'User', ActiveStatus: true, IfDeleted: false }),
        CustomersManagement.CustomerSchema.find({ "BusinessAndBranches.Business": { $in: BuyerBusiness }, CustomerType: 'User', ActiveStatus: true, IfDeleted: false }),
        BusinessAndBranchManagement.BusinessSchema.findOne({ _id: ReceivingData.Business, ActiveStatus: true, IfDeleted: false }),
        BusinessAndBranchManagement.BusinessSchema.findOne({ _id: ReceivingData.BuyerBusiness, ActiveStatus: true, IfDeleted: false }),
        InvoiceManagement.InvoiceSchema.find({ BuyerBusiness: ReceivingData.BuyerBusiness, $or: [{ InvoiceStatus: 'Pending' }, { InvoiceStatus: 'Accept' }], ActiveStatus: true, IfDeleted: false }),
        BusinessAndBranchManagement.BusinessSchema.findOne({ _id: ReceivingData.BuyerBusiness, ActiveStatus: true, IfDeleted: false }),
        InvoiceManagement.InvoiceSchema.find({ BuyerBusiness: ReceivingData.BuyerBusiness, Business: ReceivingData.Business, StatusType:'uncleared', IfUsedTemporaryCredit: false, ActiveStatus: true, IfDeleted: false }),
        InvoiceManagement.InvoiceSchema.find({ BuyerBusiness: ReceivingData.BuyerBusiness, Business: ReceivingData.Business, PaidORUnpaid: "Unpaid", IfUsedTemporaryCredit: true, InvoiceStatus: 'Accept', ActiveStatus: true, IfDeleted: false })
      ]);
    
      // Calculate total open invoice amounts
      let totalOpenInvoiceAmounts = 0;
      creditInvoices.forEach(invoice => {
        totalOpenInvoiceAmounts += invoice.ChartAmount;
      });
   
      var totalInviteAmount = invite.BuyerCreditLimit;
      // Verify if new request limit is within seller's credit limit
      if ( newRequestLimit > totalInviteAmount) { // Increase
       
        if (newRequestLimit <= sellerBusiness.AvailableCreditLimit) {
       
        if (newRequestLimit > totalOpenInvoiceAmounts) {
      
          // Update buyer business credit limits
          const updatedBuyerCreditLimit = parseFloat(ReceivingData.BuyerCreditLimit).toFixed(2);
          await BusinessAndBranchManagement.BusinessSchema.updateOne({ "_id": ReceivingData.BuyerBusiness }, {
            $set: {
              "BusinessCreditLimit": newRequestLimit,
              "AvailableCreditLimit": newRequestLimit
            }
          });
    
          //Update Invite Credit Limits
          await InviteManagement.InviteManagementSchema.updateOne({ "_id": ReceivingData.InviteId }, {
            $set: {
              "BuyerCreditLimit": updatedBuyerCreditLimit,
              "AvailableLimit": updatedBuyerCreditLimit,
              "BuyerPaymentCycle": ReceivingData.BuyerPaymentCycle,
              "BuyerPaymentType": ReceivingData.BuyerPaymentType,
            }
          });
    
          // Update seller business available credit limit (Decrease)
          var BuyerReduceAmount = 0;
          BuyerReduceAmount = buyerBusiness.AvailableCreditLimit - newRequestLimit
          sellerBusiness.AvailableCreditLimit += BuyerReduceAmount;
          await sellerBusiness.save();
            ...
            if (sellerUsers.length > 0) {
                sellerUsers.map(Obj => {
                  ...
                  var payload = {
                     notification: {
                        title: 'Hundi-Team',
                        body: invite.BuyerBusiness.FirstName + ''+invite.BuyerBusiness.LastName + ' of ' + 'credit limit has been changed to Rs.' + ReceivingData.BuyerCreditLimit + ' and Payment cycle to (' + ReceivingData.BuyerPaymentCycle + 'Days) by ' + invite.Business.FirstName + ',' + invite.Business.LastName,
                        sound: 'notify_tone.mp3'
                     },
                     data: {
                        Customer: Obj._id,
             ...
             const CreateNotificationSeller = new NotificationManagement.NotificationSchema({
                ...
                Message: invite.BuyerBusiness.FirstName + ''+invite.BuyerBusiness.LastName  + ' accepted your invite. Click here to create your first invoice for ' + invite.BuyerBusiness.FirstName + ''+invite.BuyerBusiness.LastName,
                Message_Received: true,
                Message_Viewed: false,
                ActiveStatus: true,
                IfDeleted: false,
             ...
             if (buyerUsers.length > 0) {
              buyerUsers.map(Obj => {
                  ...
                  var payload = {
                     notification: {
                        title: 'Hundi-Team',
                        body: invite.BuyerBusiness.FirstName + ''+invite.BuyerBusiness.LastName + 'credit limit has been changed to Rs.' + ReceivingData.BuyerCreditLimit + ' and Payment cycle to (' + ReceivingData.BuyerPaymentCycle + 'Days) by ' + invite.BuyerBusiness.FirstName + ''+invite.BuyerBusiness.LastName,
                        sound: 'notify_tone.mp3'
                     },
                     data: {
                        Customer: Obj._id,
                  ...
                  var SmsMessage = invite.BuyerBusiness.FirstName + ''+invite.BuyerBusiness.LastName + 'credit limit has been changed to Rs.' + ReceivingData.BuyerCreditLimit + ' and Payment cycle to (' + ReceivingData.BuyerPaymentCycle + 'Days) by ' + invite.BuyerBusiness.FirstName + ''+invite.BuyerBusiness.LastName ;
                  const params = new URLSearchParams();
                  params.append('key', '25ECE50D1A3BD6');
                  params.append('msg', SmsMessage);
                  params.append('senderid', 'TXTDMO');
             ...
             var CustomerFCMToken = [];
             CustomerFCMToken.push(invite.Buyer.Firebase_Token);
             var payload = {
                notification: {
                  title: 'Hundi-Team',
                  body: invite.BuyerBusiness.FirstName + ''+invite.BuyerBusiness.LastName + 'credit limit has been changed to Rs.' + ReceivingData.BuyerCreditLimit + ' and Payment cycle to (' + ReceivingData.BuyerPaymentCycle + 'Days) by ' + invite.BuyerBusiness.FirstName + ''+invite.BuyerBusiness.LastName,
             ...
             params.append('senderid', 'TXTDMO');
             params.append('routeid', '3');
             params.append('contacts', invite.Buyer.Mobile);
             ...
             const CreateBuyerNotification = new NotificationManagement.NotificationSchema({
                User: null,
                CustomerID: invite.BuyerBusiness._id,
                Notification_Type: 'BuyerChangedInvite',
                Message: invite.BuyerBusiness.FirstName + ''+invite.BuyerBusiness.LastName + 'credit limit has been changed to Rs.' + ReceivingData.BuyerCreditLimit + ' and Payment cycle to (' + ReceivingData.BuyerPaymentCycle + 'Days) by ' + invite.BuyerBusiness.FirstName + ''+invite.BuyerBusiness.LastName,
                Message_Received: true,
                Message_Viewed: false,
                ActiveStatus: true,
                IfDeleted: false,
          ...
          res.status(200).send({ Status: true, Message: 'Buyer Credit  limit Successfully Updated!' });
        } else {
          res.status(200).send({ Status: false, Message: "Sorry you have Open Invoices, you cannot reduce the amount" });
        }
      } else {
        // console.log("Your Requested Limit should be within the Seller Business Credit Limit");
        res.status(200).send({ Status: false, Message: "Your Requested Limit should be within the Seller Business Credit Limit" });
      }
    } else if ( newRequestLimit < totalInviteAmount) {
     
      if (newRequestLimit <= sellerBusiness.AvailableCreditLimit) {
       
        // Verify if new request limit is within available credit limit considering open invoices
        if (newRequestLimit > totalOpenInvoiceAmounts) {
      ...
      res.status(200).send({ Status: false, Message: "Enter a Valid Busines Credit Limit!" });
    } else {
      res.status(417).send({ Status: false, Message: "Some Ocurred error!" });
    }
    } catch (error) {
                    ...
                    if (result.length !== 0) {
                      ...
                      if (ReceivingData.Invite_Status === '') {
                        ...
                        if (ResponsePending_Approval.length > 0) {
                          ...
                          res.status(200).send({ Status: true, Message: "You Doesn't having any Customer!", InviteTitle: 'Pending_Approval', Response: [] });
                        }
                      } else {
                        res.status(200).send({ Status: true, InviteTitle: ReceivingData.Invite_Status, Message: 'Your Connected Customers', Response: result });
                      }
                    } else {
                      res.status(200).send({ Status: true, InviteTitle: 'Pending_Approval', Message: "You Doesn't having any Customer!", Response: [] });
                    }
                  }
                });
            } else {
...
exports.SellerAgainstBusinessList = function (req, res) {
  var ReceivingData = req.body;
  // console.log('ReceivingData seller against business list', ReceivingData);
  if (!ReceivingData.Seller || ReceivingData.Seller === '') {
    res.status(400).send({ Status: false, Message: "Seller can not be empty" });
  } else if (!ReceivingData.CustomerCategory || ReceivingData.CustomerCategory === '') {
    res.status(400).send({ Status: false, Message: "Customer Category can not be empty" });
...
exports.BuyerAgainstBusinessList = function (req, res) {
  var ReceivingData = req.body;
  // console.log('received data buyeragainstbusnesslist', ReceivingData);
  if (!ReceivingData.Buyer || ReceivingData.Buyer === '') {
    res.status(400).send({ Status: false, Message: "Buyer can not be empty" });
  }  else if (!ReceivingData.Business || ReceivingData.Business === '') {
    res.status(400).send({ Status: false, Message: "Business Details can not be empty" });
