         ...
         var InvoiceDetails = JSON.parse(JSON.stringify(Response[0]));
            if (InvoiceDetails !== null && InvoiceDetails.length > 0 ) {
               InvoiceDetails.forEach((element)=>{
                  ...
                  var BuyerBusinessRecieve = JSON.stringify(ReceivingData.BuyerBusiness);
                  var BuyerBusinessExist = JSON.stringify(element.BuyerBusiness);
                 if (element.InvoiceNumber === ReceivingData.InvoiceNumber && SellerRecieve === SellerExist && BusinessRecieve === BusinessExist && BuyerBusinessRecieve === BuyerBusinessExist) {
...
exports.InvoiceNumberDuplicateCheckNew = async function (req, res) {
   var ReceivingData = req.body;
   if (!ReceivingData.InvoiceNumber || ReceivingData.InvoiceNumber === '') {
      res.status(400).send({ Status: false, Message: "Invoice Number can not be empty" });
   } else {
      Promise.all([
         InvoiceManagement.InvoiceSchema.find({ InvoiceNumber: ReceivingData.InvoiceNumber }, {}, {}).exec()
      ]).then(Response => {
         var InvoiceDetails = JSON.parse(JSON.stringify(Response[0]));
         if (InvoiceDetails !== null && InvoiceDetails.length > 0) {
            res.status(200).send({ Status: false, Message: "Invoice Number Already Exist!" });
         } else {
            res.status(200).send({ Status: true, Message: "Invoice Number Not Found!" });
         }
      }).catch(Error => {
   ...
   if (!ReceivingData.Seller || ReceivingData.Seller === '') {
      ...
      Promise.all([
         ...
         if (InviteDetails.length > 0) {
            ...
            // if receivingdata.invoiceattachments is not empty then find if there is any base64 data and convert it to image and save it in the folder
            if (ReceivingData.InvoiceAttachments.length !== 0 && ReceivingData.NewImages === true) {
               
               const InvoiceAttachments = ReceivingData.InvoiceAttachments.map(InvObj => {
                  var InvoiceObj = {
                     'fileName': '',
                     'fileType': ''
                  };
                  var RandomNumber = Math.floor(Math.random() * 6873887587587);
                  var reportData = InvObj.replace(/^data:[a-z]+\/[a-z]+;base64,/, "").trim();
                  var buff = Buffer.from(reportData.replace(/\r?\n|\r/g, " "), 'base64');
                  const fineName = 'Uploads/Invoice/' + SellerDetails._id + RandomNumber + '.png';

                  InvoiceObj.fileName = SellerDetails._id + RandomNumber + '.png';

                  InvoiceObj.fileType = 'image/png';

                  fs.writeFileSync(fineName, buff);

                  return InvoiceObj;
               });
               Promise.all(InvoiceAttachments).then((newImages) => {
            ...
            if (ReceivingData.InvoiceStatus === "Disputed") {
               ...
               InvoiceDetails.save((err, result) => {
                  if (err) {
                     ...
                     const Create_Invoice = new InvoiceManagement.InvoiceSchema({
                        ...
                        PaidORUnpaid: "Unpaid",
                        StatusType: "uncleared",
                        InvoiceAttachments: newImages,
                        ActiveStatus: true,
                        IfDeleted: false
                     });

                     // console.log('Create_Invoice',Create_Invoice);
                     Create_Invoice.save((errNew, resultNew) => {
                        ...
                        if (errNew) {
                           ...
                           res.status(417).send({ Status: false, Message: "Some errors predicted, We are unable to create the Invoice after create!.", Error: errNew });
                        } else {
               ...
               InvoiceDetails.InvoiceAttachments = newImages;
               InvoiceDetails.InvoiceDescription = ReceivingData.InvoiceDescription;
               InvoiceDetails.Remarks = ReceivingData.Remarks;

               // console.log('InvoiceDetails',InvoiceDetails);

               InvoiceDetails.updateOne(InvoiceDetails).exec((err, result) => {
                  if (err) {
                     ErrorHandling.ErrorLogCreation(req, 'Invoice Update Error', 'InvoiceManagement.Controller -> Invoice_Update', JSON.stringify(err));
                     res.status(417).send({ Status: false, Message: "Some errors predicted, We are unable to create the Invoice!.", Error: err });
                  } else {
      ...
      CustomersManagement.CustomerSchema.findOne({ _id: ReceivingData.CustomerId, ActiveStatus: true, IfDeleted: false }, {}, {}).exec(function (err, result) {
         if (err) {
            ...
            if (result !== null) {
               ...
               var mainQuery = {};
               if (ReceivingData.FilterQuery.StatusType === 'OverDue' || ReceivingData.FilterQuery.StatusType === 'DueToday' || ReceivingData.FilterQuery.StatusType === 'Upcoming') {
                  mainQuery = { IfDeleted: false, ActiveStatus: true,StatusType:'uncleared'}; //
               }  else if (ReceivingData.FilterQuery.StatusType === 'All' || ReceivingData.FilterQuery.StatusType === ''){
                   mainQuery = { IfDeleted: false, ActiveStatus: true}; //PaidORUnpaid:'Unpaid'
               }

               if (ReceivingData.FilterQuery.StatusType === 'OverDue' || ReceivingData.FilterQuery.StatusType === 'DueToday' || ReceivingData.FilterQuery.StatusType === 'Upcoming') {
                  var SubQueryOne = { IfDeleted: false, ActiveStatus: true, InvoiceStatus: "Pending",StatusType:'uncleared' };
                  var SubQueryTwo = { IfDeleted: false, ActiveStatus: true, InvoiceStatus: "Accept",StatusType:'uncleared' };
                  var SubQueryThree = { IfDeleted: false, ActiveStatus: true, InvoiceStatus: "Disputed",StatusType:'uncleared' };
               } else if (ReceivingData.FilterQuery.StatusType === 'All' || ReceivingData.FilterQuery.StatusType === ''){
               ...
               if (ReceivingData.FilterQuery.DateRange === 'Custom' && ReceivingData.FilterQuery.CustomDateRange.From === ReceivingData.FilterQuery.CustomDateRange.To) {
                  customRange.to = new Date(new Date(customRange.to).setHours(23, 59, 59, 999));
                  customRange.from = new Date(new Date(customRange.from).setHours(0, 0, 0, 0));
               ...
               if (ReceivingData.FilterQuery.StatusType === 'OverDue') {
                  ...
                  // set fromdate one year back from today
                  FromDate = new Date(new Date(todayRange.from).setFullYear(new Date(todayRange.from).getFullYear() - 10));
                  }
               }
               if (ReceivingData.FilterQuery.StatusType === 'Upcoming') {
                  ...
                  if ( ToDate !== null && new Date(ToDate).valueOf() <= new Date(FromDate).valueOf()  ) {
                     ...
                     // set todate one year ahead from today
                     ToDate = new Date(new Date(todayRange.to).setFullYear(new Date(todayRange.to).getFullYear() + 10));
                  }
               ...
               if (FromDate !== null && ToDate !== null) {
                  if (FromDate === null || FromDate === null) {
                     mainQuery.InvoiceDueDate = ToDate === null ? {$gte: FromDate} : FromDate === null ? {$lte: ToDate} : null;
                     
                     SubQueryOne.InvoiceDueDate = mainQuery.InvoiceDueDate;
                     SubQueryTwo.InvoiceDueDate = mainQuery.InvoiceDueDate;
                     SubQueryThree.InvoiceDueDate = mainQuery.InvoiceDueDate;
                  } else 
                  {
                     if (ToDate === null) 
                     {
                        mainQuery["$and"] = [{InvoiceDueDate: {$gte: FromDate} }];
                        SubQueryOne["$and"] = [{InvoiceDueDate: {$gte: FromDate} }];
                        SubQueryTwo["$and"] = [{InvoiceDueDate: {$gte: FromDate} }];
                        SubQueryThree["$and"] = [{InvoiceDueDate: {$gte: FromDate} }];
                     }
                     ...
                     else
                     {
                        ...
                        mainQuery["$and"] = [{InvoiceDueDate: {$gte: FromDate} }, {InvoiceDueDate: {$lte: ToDate} }];
                        SubQueryOne["$and"] = [{InvoiceDueDate: {$gte: FromDate} }, {InvoiceDueDate: {$lte: ToDate} }];
                        SubQueryTwo["$and"] = [{InvoiceDueDate: {$gte: FromDate} }, {InvoiceDueDate: {$lte: ToDate} }];
                        SubQueryThree["$and"] = [{InvoiceDueDate: {$gte: FromDate} }, {InvoiceDueDate: {$lte: ToDate} }];
                     }
               ...
               Promise.all([
                  ...
                  if (ReceivingData.FilterQuery.StatusType === 'OverDue' || ReceivingData.FilterQuery.StatusType === 'DueToday' || ReceivingData.FilterQuery.StatusType === 'Upcoming') {
                      OpenCount = response[0];
                      AcceptCount = response[1];
                      DisputeCount = response[2];
                  } else if(ReceivingData.FilterQuery.StatusType === 'All'  || ReceivingData.FilterQuery.StatusType === '') {
                      OpenCount = response[0];
                      AcceptCount = response[1];
                      DisputeCount = response[2];
                  ...
                  Promise.all([
                     ...
                     if (ReceivingData.FilterQuery.StatusType === 'OverDue' || ReceivingData.FilterQuery.StatusType === 'DueToday' 
                        || ReceivingData.FilterQuery.StatusType === 'Upcoming') {
                        responseNew[0] = responseNew[0].filter(Obj => Obj.StatusType === 'uncleared').map(Obj => {
                          var PaymentProcess = Obj.PaidAmount === Obj.InvoiceAmount ? 'PaymentCompleted' 
                            ...
                            : Obj.InvoiceAmount === (Obj.RemainingAmount + Obj.InProgressAmount) ? 'WaitForPayment' 
                            : 'PartialPayment';
                          Obj.PaymentStatus = Obj.PaymentStatus !== undefined ? Obj.PaymentStatus : PaymentProcess;
                      
                          var invDate = new Date(Obj.InvoiceDueDate);
                          var conFrom = todayRange.from;
                          var conTo = todayRange.to;
                      
                          var InvoiceStatus = (invDate >= conFrom && invDate <= conTo) && PaymentProcess !== 'PaymentCompleted' ? 'DueToday' 
                            : (invDate > conTo && PaymentProcess !== 'PaymentCompleted') ? 'Upcoming' 
                            : (invDate < conFrom && PaymentProcess !== 'PaymentCompleted') ? 'OverDue' 
                            : '';
                     ...
                     } else if (ReceivingData.FilterQuery.StatusType === 'All') {
                        responseNew[0] = responseNew[0].map(Obj => {
                           ...
                           Obj.StatusType = InvoiceStatus;
                           // console.log(InvoiceStatus,'InvoiceStatusInvoiceStatus');
                           // console.log(Obj.StatusType,);
                           return Obj;
                        });
                     }
                     ...
                     var findActiveTab = null 

                     if (ReceivingData.FilterQuery.StatusType === 'OverDue' || ReceivingData.FilterQuery.StatusType === 'DueToday' || ReceivingData.FilterQuery.StatusType === 'Upcoming') {
                        findActiveTab = DisputeCount > 0 ? "Disputed" : OpenCount > 0 ? "Pending" :  "Accept";
                     }
                     ...
                     const Response = {
                        ...
                        ActiveTab: ReceivingData.InvoiceType ? ReceivingData.InvoiceType : findActiveTab 
                     };
...
exports.InvoiceDetails = function (req, res) {
   ...
   if (!ReceivingData.Invoice || ReceivingData.Invoice === '') {
      ...
      InvoiceManagement.InvoiceSchema.findOne({
         ...
         .populate({ path: 'Business', select: ['FirstName','LastName', 'BusinessCreditLimit', 'AvailableCreditLimit','BusinessUnique_id'] })
         .populate({ path: 'BuyerBusiness', select: ['FirstName','LastName', 'BusinessCreditLimit', 'AvailableCreditLimit','BusinessUnique_id'] })
            ...
            if (err) {
               ...
               if (result !== null) {
                  Promise.all([
                     ...
                     InvoiceManagement.InvoiceSchema.find({ InvoiceStatus:'Pending',Buyer:result.Buyer._id,BuyerBusiness:result.BuyerBusiness._id,
                        ActiveStatus:true,IfDeleted:false},{},{}).exec(),
                     ...
                     var EditOpenInvoiceDetails = JSON.parse(JSON.stringify(Response[1]));
                     var OpenInvoiceDetails = JSON.parse(JSON.stringify(Response[2]));
                     var InviteDetails = JSON.parse(JSON.stringify(Response[3]));
                        
                    
                     if (BuyerBusinessDetails !== null && EditOpenInvoiceDetails !== null && InviteDetails !== null) {
                        var InviteAvailableAmount = 0;
                        var InvoiceTotalAmount = 0;
                        var OpenInvoiceTotalAmount = 0;
                        ...
                        // console.log('OpenInvoiceTotalAmount:',OpenInvoiceTotalAmount);
                        // console.log('InviteAvailableAmount:',InviteAvailableAmount);
                         if (IfUsedTemporaryCredit === false) {
                           CheckInvoiceAmounts = InviteAvailableAmount - OpenInvoiceTotalAmount;
                        } 
...
exports.BuyerInvoice_Dispute = function (req, res) {
   ...
   //  return
   if (!ReceivingData.InvoiceId || ReceivingData.InvoiceId.length === 0) {
         ...
         if (CustomerDetails !== null && InvoiceDetails.length !== 0) {
            InvoiceManagement.InvoiceSchema.updateMany(
               ...
               if (err_1) {
                  ...
                  InvoiceDetails.map(obJ=>{
                     if (obJ.InvoiceAttachments.length !== 0) {
                        var InvoiceArr = [];
                        var InvoiceAttachments = obJ.InvoiceAttachments;
                        InvoiceAttachments = InvoiceAttachments.map(Obj => {
                           var InvoiceObj = {
                              _id: String,
                              fileName: String,
                        ...
                        InvoiceManagement.InvoiceSchema.updateOne({ _id: obJ._id }, { InvoiceAttachments: InvoiceArr }).exec();
                     }
                  });
      ...
      for (const invoice of InvoiceDetails) {
         ...
         let updateQuery = {
            $set: {
               ...
               "IfBuyerApprove": true,
               "IfBuyerNotify": true,
               "ChartAmount": invoice.InvoiceAmount
            }
         };
...
exports.Web_BuyerInvoice_Accept = function (req, res) {
   var ReceivingData = req.body;
   if (!ReceivingData.WaitForApprovalArray || ReceivingData.WaitForApprovalArray.length === 0) {
       res.status(400).send({ Status: false, Message: "InvoiceId can not be empty" });
   } else if (!ReceivingData.Buyer || ReceivingData.Buyer === '') {
       res.status(400).send({ Status: false, Message: "Buyer can not be empty" });
   } else {
...
//     // Set the HTML content using a template string
//     const htmlContent = `
//       <html lang="en">
//   <head>
//     <meta charset="UTF-8" />
...
//       <tr class="header">
//         <td>
//           <h2>${payload.comapanyDeatils}</h2>
//           <span>ISO 9001: 2015 Certified Labs</span>
//           <p>GSTIN :${payload.companyGSTIn}</p>
//           <p>State: ${payload.stateName}</p>
//           <p>State Code: ${payload.stateCode}</p>
//         </td>
//         <td colspan="2"><p>${payload.companyAddress}</p></td>
//       </tr>
//       <tr>
//         <td><p>to</p></td>
...
//     if(base64Pdf) {
//    const ecgIndex = mongoose.Types.ObjectId();
//    const Create_Base64 = new InvoiceManagement.Base64Schema({
//       _id: ecgIndex,
//       Base64:  'data:image/jpeg;base64,' + base64Pdf,
//       Active_Status: true,
//       If_Deleted: false
//    });
//    Create_Base64.save();	
...
//    res.status(200).send({ message: 'PDF generated successfully and stored in the database!', resData: base64Pdf })
//   } catch (err) {
...
// // Getting the Base64pdf from db and pass it into front end
// exports.getBase64Pdf = async (req, res) => {
//    try {
//       const base64Id = req.body.id;
//       const base64Pdf = await InvoiceManagement.Base64Schema.findById(base64Id).select('Base64');
//       if(base64Pdf) {
//           res.status(200).send({ base64Pdf: base64Pdf.Base64 });
//       } else {
//          res.status(404).send({ Message: 'Base64 PDF not Found' });
//       }
//    } catch (err) {
//       res.status(500).send({ Message: 'Error retriving base64', err })
//    }
// }
