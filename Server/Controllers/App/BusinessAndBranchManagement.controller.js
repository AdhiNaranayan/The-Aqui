...
exports.CreateBusiness = function (req, res) {
   ...
   if (!ReceivingData.CustomerId || ReceivingData.CustomerId === '') {
   ...
   } else if (!ReceivingData.BusinessUnique_id || ReceivingData.BusinessUnique_id === '') {
      res.status(400).send({ Status: false, Message: "Business Unique Id can not be empty" });
      ...
      // find if business unique id already exists in business collection
      BusinessAndBranchManagement.BusinessSchema.findOne({ BusinessUnique_id: ReceivingData.BusinessUnique_id, ActiveStatus: true, IfDeleted: false }, {}, {}).exec()
         .then(BusinessResponse => {
            // console.log(BusinessResponse, 'BusinessResponse');
            if (BusinessResponse !== null) {
               res.status(200).send({ Status: false, Message: 'Business Unique Id already exists. Please choose different ID.' });
               return;
            } 
         }).catch(error => {
      ...
      Promise.all([
         ...
         if (ResponseRes[0] !== null) {
            ...
            const Create_Business = new BusinessAndBranchManagement.BusinessSchema({
               ...
               BusinessUnique_id:ReceivingData.BusinessUnique_id,
            ...
            Promise.all([
               ...
               BusinessAndBranchManagement.BusinessSchema.find({ActiveStatus:true,IfDeleted:false,Mobile:ReceivingData.Mobile},{},{}).exec()
                     ...
                     var RFirstName = String(ReceivingData.FirstName).trim();
                     var RLastName = String(ReceivingData.LastName).trim();
                     var DFirstName;
                     var DLastName;
                  
                     var isDuplicate = false;
                     var IdisDuplicate = false;

                     BusinessDetails.forEach(obj => {
                        var DFirstName = obj.FirstName.trim();
                        var DLastName = obj.LastName.trim();
                        
                        var BusinessId = obj.BusinessUnique_id
                        ...
                        if (ReceivingData.BusinessUnique_id === BusinessId) {
                           IdisDuplicate = true;
                        }
                     });
                     
                     if (isDuplicate) {
                        res.status(200).send({ Status: false, Message: 'Mobile number already registered for this business name' });
                     } else if (IdisDuplicate) {
                        res.status(200).send({ Status: false, Message: 'Try with Different Business Id Already Exist!' });
                     } else {
                        Create_Business.save()
                           .then(() => {
...
exports.Invite_BusinessDetailsList = function (req, res) { 
   ...
   if (!ReceivingData.CustomerId || ReceivingData.CustomerId === '') {
      ...
      Promise.all([
         ...
         var BusinessDetails = [];
         BusinessDetails = JSON.parse(JSON.stringify(Response[0]));
         var CustomerUserDetails = JSON.parse(JSON.stringify(Response[1]));
         var InvitedDetails = JSON.parse(JSON.stringify(Response[2]));
      ...
      if (CustomerUserDetails.CustomerType === 'User') {
         Promise.all([
            BusinessAndBranchManagement.BusinessSchema.find({Customer:CustomerUserDetails.Owner,IfSeller: IfSeller, IfBuyer: IfBuyer,ActiveStatus:true,IfDeleted:false},{},{})
            .populate({ path: "Customer", select: ["ContactName", "Mobile", "Email", "CustomerCategory", "CustomerType"] })
            .populate({ path: "Industry", select: ["Industry_Name", "Status"] })
            .exec(),
            InviteManagement.InviteManagementSchema.find({
               InvitedBy: CustomerUserDetails.Owner,
               $or: [{ Invite_Status: 'Pending_Approval' }, { Invite_Status: 'Pending' }],
               ActiveStatus: true, IfDeleted: false
           }, {}, {}).exec()
          ]).then(Response => {
...
exports.BusinessUpdate = function (req, res) {
   ...
   ReceivingData.BusinessId = mongoose.Types.ObjectId(ReceivingData.BusinessId);
      ...
      var OpenInviteQuery = {};
         ...
         InviteQuery = {Seller:ReceivingData.CustomerId,Business:ReceivingData.BusinessId,Invite_Status:'Accept', ActiveStatus:true,IfDeleted:false}
         OpenInviteQuery = {Seller:ReceivingData.CustomerId,Business:ReceivingData.BusinessId, Invite_Status:'Pending_Approval', ActiveStatus:true,IfDeleted:false}
   ...
   Promise.all([
      ...
      // InviteManagement.InviteManagementSchema.find({Business:ReceivingData.BusinessId,ActiveStatus: true, IfDeleted: false},{},{}).exec(),
      InviteManagement.InviteManagementSchema.find(InviteQuery,{},{}).exec(),
      InviteManagement.InviteManagementSchema.find(OpenInviteQuery,{},{}).exec(),
      InvoiceManagement.InvoiceSchema.find({Business:ReceivingData.BusinessId,StatusType:'uncleared',ActiveStatus:true,IfDeleted:false},{},{}).exec()
   ]).then(Response => {
      ...
      var OpenInviteDetails = JSON.parse(JSON.stringify(Response[3]));
      var InvoiceDetails = JSON.parse(JSON.stringify(Response[4]));
      ...
      if (CustomerDetails !== null && BusinessDetails !== null ) {
         ...
         var totalInvitedLimit = 0;
         var totalOpenInvitedLimit = 0;
         var AllInviteAmounts = 0;
         var NewCreditLimit = Number(ReceivingData.BusinessCreditLimit);

        
         InviteDetails.map(oBj =>{
            // console.log(oBj);
            totalInvitedLimit += oBj.BuyerCreditLimit;
         });
         
         OpenInviteDetails.map(Obj=>{
            totalOpenInvitedLimit += Obj.BuyerCreditLimit;
         })
         
         
         AllInviteAmounts = totalInvitedLimit + totalOpenInvitedLimit;
        
         var errorMessage, errorStatus ,StatusCode;
       
   
         // if (InvoiceDetails.length === 0) {

         if (NewCreditLimit > AllInviteAmounts) {
               if (NewCreditLimit !== BusinessDetails.BusinessCreditLimit) {
                  BusinessCreditLimit = NewCreditLimit;
                  var Avail = NewCreditLimit - totalInvitedLimit;
                  AvailableBusinessCreditLimit = Avail;
               } 
               // else {
               //    errorMessage = "Enter a valid Business Credit Limit!.";
               //    errorStatus = 200;
               // }
         } else if (NewCreditLimit < AllInviteAmounts) {
               errorMessage = "Sorry You cannot edit your business amount you have invited to respective amount!.";
               errorStatus = false;
               StatusCode = 200;
         } else if (NewCreditLimit === AllInviteAmounts) {
            if (NewCreditLimit !== BusinessDetails.BusinessCreditLimit) {
         ...
         // } else {
         //    errorMessage = "Sorry You cannot edit your business amount you have invoices open!";
         //    errorStatus = 400;
         // }
      

         if (errorMessage) {
            res.status(StatusCode).send({ Status: errorStatus, Message: errorMessage });
         } else {
...
exports.DuplicateBusinessId = async function (req, res) {
   try {
      var ReceivingData = req.body;
      if (!ReceivingData.CustomerId || ReceivingData.CustomerId === '') {
         res.status(400).send({ Status: false, Message: "Customer can not be empty" });
         ...
         // Check if business unique id already exists
         const existingBusiness = await BusinessAndBranchManagement.BusinessSchema.findOne({ Mobile: ReceivingData.Mobile, BusinessUnique_id: ReceivingData.BusinessUnique_id });

         if (existingBusiness) {
             return res.status(200).send({ Status: false, Message: 'Business Unique ID already exists. Please choose a different ID.' });
         } else {
             return res.status(200).send({ Status: false, Message: 'No Business exists with this ID!' });
         }
      ...
      Promise.all([
         ...
         if (CustomerDetails !== null) {
            ...
            if (ReceivingData.CustomerCategory === 'Seller') {
               if (CustomerDetails.CustomerType === 'Owner') {
                  InvoiceQuery = { Seller: ReceivingData.Customer, PaidORUnpaid: "Unpaid",InvoiceStatus: "Accept", ActiveStatus: true, IfDeleted: false };
                  ChartInvoiceQuery = { Seller: ReceivingData.Customer,InvoiceStatus: "Accept", ActiveStatus: true, IfDeleted: false };
                  FindQuery = { Customer: ReceivingData.Customer, IfSeller: true, IfBuyer: false, ActiveStatus: true, IfDeleted: false };
                  InviteQuery = { Seller: ReceivingData.Customer, Invite_Status: 'Accept', ActiveStatus: true, IfDeleted: false };
                  TemporaryQuery = { Seller: ReceivingData.Customer, Request_Status: 'Accept', ActiveStatus: true, IfDeleted: false };
               } else if (CustomerDetails.CustomerType === 'User') {
               ...
               if (CustomerDetails.CustomerType === 'Owner') {
                  InvoiceQuery = { Buyer: ReceivingData.Customer, PaidORUnpaid: "Unpaid",InvoiceStatus: "Accept", ActiveStatus: true, IfDeleted: false };
                  ChartInvoiceQuery = { Buyer: ReceivingData.Customer,InvoiceStatus: "Accept", ActiveStatus: true, IfDeleted: false };
                  InvoicePendingListQuery =  { Buyer: ReceivingData.Customer,  InvoiceStatus: "Pending", ActiveStatus: true, IfDeleted: false };
                  FindQuery = { Customer: ReceivingData.Customer, IfSeller: false, IfBuyer: true, ActiveStatus: true, IfDeleted: false };
                  TemporaryQuery = { Buyer: ReceivingData.Customer, Request_Status: 'Accept', ActiveStatus: true, IfDeleted: false };
                  InviteQuery = { Buyer: ReceivingData.Customer, Invite_Status: 'Accept', ActiveStatus: true, IfDeleted: false };
            ...
            Promise.all([
               ...
               InvoiceManagementModel.InvoiceSchema.find(InvoicePendingListQuery, {}, {}).exec(),
               InvoiceManagementModel.InvoiceSchema.find(ChartInvoiceQuery, {}, {}).exec(),
            ]).then(ResponseRes => {
               ...
               var ChartInvoiceAmounts = JSON.parse(JSON.stringify(ResponseRes[6]));
               ...
               if (BusinessDetails.length > 0) {
                  ...
                  BusinessDetails.map(Obj => {
                     ...
                     const result4Arr = InvoiceDetails.filter(obj1 => (obj1.BuyerBusiness === Obj._id && Obj.IfBuyer && !Obj.IfSeller) ||
                        ...
                        const ChartArr = ChartInvoiceAmounts.filter(obj1 => (obj1.BuyerBusiness === Obj._id && Obj.IfBuyer && !Obj.IfSeller) ||
                        (obj1.Business === Obj._id && !Obj.IfBuyer && Obj.IfSeller));
                     ...
                     // if (result4Arr.length > 0) {
                        ...
                        if (ChartArr) {
                           ChartArr.map(chart=>{
                              ...
                              InvoiceAmount = parseFloat(InvoiceAmount) + parseFloat(chart.RemainingAmount); //parseFloat(obj.AvailableAmount);
                              OverDueDate = new Date(chart.InvoiceDueDate);
                              DueTodayDate = new Date(chart.InvoiceDueDate);
   
                              // Parse invoice due date
                           var mInvoiceDate = moment(chart.InvoiceDueDate).startOf('day'); // Consider using moment to ensure consistent date comparison
                           
                           //    if (chart.ChartAmount) {
                           // // Compare due date with today's date
                           //       if (mInvoiceDate.isBefore(TodayDate, 'day')) {
                           //          OverDueAmount += parseFloat(chart.ChartAmount);
                           //       } else if (mInvoiceDate.isAfter(TodayDate, 'day')) {
                           //          UpComingAmount += parseFloat(chart.ChartAmount);
                           //          console.log(UpComingAmount);
                           //       } else if (mInvoiceDate.isSame(TodayDate, 'day')) {
                           //          DueTodayAmount += parseFloat(chart.ChartAmount);
                           //       }
                           //    } else {
                           //       OverDueAmount = 0;
                           //       UpComingAmount = 0;
                           ...
                           //    }
                              ...
                              if (chart.ChartAmount) {
                                 if (mInvoiceDate.isBefore(TodayDate, 'day')) {
                                   OverDueAmount = parseFloat(OverDueAmount) + parseFloat(chart.ChartAmount);
                               } else if (mInvoiceDate.isSame(TodayDate, 'day')) {
                                   DueTodayAmount = parseFloat(DueTodayAmount) + parseFloat(chart.ChartAmount);
                               } else {
                                     if (chart.ChartAmount != 0) {      
                                       UpcomingChartAmounts += chart.ChartAmount ;
                                       // console.log(UpcomingChartAmounts,'234234');
                                       UpComingAmount = parseFloat(UpcomingChartAmounts);
                                     } else if (chart.ChartAmount === 0) {
                                       UpComingAmount = 0;
                                     }
                        ...
                        result4Arr.map(obj => {
                           ...
                           Obj.OverDueAmount = parseFloat(Obj.OverDueAmount);
                           Obj.UpComingAmount = UpComingAmount.toFixed(2);
                           Obj.UpComingAmount = parseFloat(UpcomingChartAmounts);
                           Obj.DueTodayAmount = DueTodayAmount.toFixed(2);
                           Obj.DueTodayAmount = parseFloat(Obj.DueTodayAmount);
...
exports.IndustrySimpleListMobile = function (req, res) {
   //var ReceivingData = req.query;
   IndustryManagement.IndustrySchema.find({ Status: 'Activated' }, {}, {}, function (err, result) {
      if (err) {
          ErrorHandling.ErrorLogCreation(req, 'Industry Find error', 'BusinessManagement -> IndustrySimpleList', JSON.stringify(err));
          res.status(417).send({ Http_Code: 417, Status: false, Message: "Some error occurred while Find the User Management!.", Error: err });
...
exports.MyBusinessListMobile = function (req, res) {
   var ReceivingData = req.query;
