...
async function createPayment() {
   ...
   //      var LastPayment_Reference = LastPayment !== null ? (LastPayment.PaymentID_Unique + 1) : 1;
     ...
     // Fetch or initialize the counter for the buyer and buyer business combination
     const counter = await PaymentManagement.PaymentSchema.findOneAndUpdate(
      { Buyer: ReceivingData.Buyer, BuyerBusiness: ReceivingData.BuyerBusiness },
      { $inc: { PaymentID_Unique: 1 } },
      { new: true, upsert: true }
   );

   const LastPayment_Reference = counter.PaymentID_Unique;
     ...
     // use await before saving the document
      ...
      //                      console.log(Obj_Invoice,'Obj_InvoiceObj_Invoice');
      //             ReceivingData.InvoiceDetails.map(Obj => {
      //                            console.log(Obj,'222');
      ...
      //                console.log(mInProgressAmount,'mInProgressAmount');
      //                   //  RemainingAmount : Obj.RemainingAmount,
      ...
      if (InvoiceDetails.length !== 0) {
         InvoiceDetails.forEach(Obj_Invoice => {
            
             const correspondingInvoice = ReceivingData.InvoiceDetails.find(Obj => Obj.InvoiceId === Obj_Invoice._id.toString());
             if (correspondingInvoice) {
                 
                 var mInProgressAmount = Number(Obj_Invoice.InProgressAmount) + Number(correspondingInvoice.InProgressAmount);
                 var x = Number(Obj_Invoice.RemainingAmount) + Number(correspondingInvoice.RemainingAmount);
                 var mRemainingAmount = Number(correspondingInvoice.InvoiceAmount) - Number(Obj_Invoice.PaidAmount) - mInProgressAmount;
            
                 // Update your invoice details here
                        InvoiceManagement.InvoiceSchema.updateOne(
                           { _id: correspondingInvoice.InvoiceId },
                           { $set: 
                              { RemainingAmount : correspondingInvoice.RemainingAmount, 
                                InProgressAmount  : mInProgressAmount,
                                PaidORUnpaid : correspondingInvoice.PaidORUnpaid}}).exec();
...
exports.PaymentDetailsUpdate = function (req, res) {
   ...
   if (!ReceivingData.Seller || ReceivingData.Seller === '') {
      ...
      Promise.all([
         ...
         var EditRemaining = 0;
         var EditInProgress = 0;
         //Invoice Update 
         Invoice.map(obj => {
          
         // mSetRemainingAmount = obj.InvoiceAmount - obj.PaidCurrentCreditAmount - obj.InProgressAmount;//500 - 0 -300
         // mSetRemainingAmount = obj.RemainingAmount -  obj.InProgressAmount;//1000-400-200

          InvoiceManagement.InvoiceSchema.findOne({_id:obj.InvoiceId,ActiveStatus:true,IfDeleted:false},{},{}).exec(
            (err,result) =>{
               if (err) {
                  console.log('When Finding Invoice We found an Error',err);
               } else {
                  var InvoiceList = result;
                 if (Number(obj.InProgressAmount) === PaymentDetails.PaymentAmount) {
                  InvoiceManagement.InvoiceSchema.updateOne(
                     { _id: obj.InvoiceId } ,
                     {
                         $set: {
                 ...
                 } else if (Number(obj.InProgressAmount) !== PaymentDetails.PaymentAmount) {
                  
               //InProgress Calculations
                  // var LastPaymentAmount = Number(PaymentDetails.PaymentAmount);
                  // var ExistingInProgressValue = Number(InvoiceList.InProgressAmount);
                  
                  // var CurrentPaymentAmount = Number(obj.InProgressAmount);
                  // var SetInProgress = LastPaymentAmount - CurrentPaymentAmount;

                  // EditInProgress = ExistingInProgressValue - SetInProgress; //590 -400 = 190 

                  var LastPaymentAmount = 0;

                  if (PaymentDetails.InvoiceDetails.length === 1) {
                   LastPaymentAmount = Number(PaymentDetails.PaymentAmount);
                  } else if (PaymentDetails.InvoiceDetails.length > 1) {
                     PaymentDetails.InvoiceDetails.map(obj=>{
                        if (InvoiceList._id.toString() === obj.InvoiceId.toString()) {
                              LastPaymentAmount = Number(obj.InProgressAmount);
                        }
                     })
                  }
                  var ExistingInProgressValue = Number(InvoiceList.InProgressAmount);
                  var ExistingRemainingAmount = Number(InvoiceList.RemainingAmount)
                  var CurrentPaymentAmount = Number(obj.InProgressAmount); //400
  
                  var SetInProgress = 0;
                  var SetInRemain = 0;
                  
                  if (ExistingInProgressValue > LastPaymentAmount) {
                     var SetInProgress = ExistingInProgressValue - LastPaymentAmount; //590-500 = 90 
                  } else if (ExistingInProgressValue < LastPaymentAmount) {
                     var SetInProgress = LastPaymentAmount - ExistingInProgressValue; //590-500 = 90
                  }
                   
                  if (PaymentDetails.InvoiceDetails.length === 1) {
                     EditInProgress = SetInProgress + CurrentPaymentAmount; //590-400 = 190 
                  } else if (PaymentDetails.InvoiceDetails.length > 1) {
                     EditInProgress = SetInProgress + CurrentPaymentAmount; //590-400 = 190 
                  }

                  if (LastPaymentAmount > CurrentPaymentAmount) {
                     SetInRemain = LastPaymentAmount - CurrentPaymentAmount ;
                  } else  if (LastPaymentAmount < CurrentPaymentAmount) {
                     SetInRemain = CurrentPaymentAmount - LastPaymentAmount ;
                  }
                  // console.log(LastPaymentAmount,'LastPaymentAmount',InvoiceList.InvoiceNumber);
                  // console.log(CurrentPaymentAmount,'CurrentPaymentAmount',InvoiceList.InvoiceNumber);
                  // console.log('------------------------------------------------');
                  EditRemaining = ExistingRemainingAmount + SetInRemain;
                  // console.log(ExistingInProgressValue,'ExistingInProgressValue',InvoiceList.InvoiceNumber);
                  // console.log(ExistingRemainingAmount,'ExistingRemainingAmount',InvoiceList.InvoiceNumber);
                  // console.log('-----------------------------');
                  // console.log(SetInRemain,'SetInRemain',InvoiceList.InvoiceNumber);
                  // console.log('=============================');
                  // console.log(EditRemaining,'EditRemaining',InvoiceList.InvoiceNumber);
                  // console.log(EditInProgress,'EditInProgress',InvoiceList.InvoiceNumber);
                  ...
                  InvoiceManagement.InvoiceSchema.updateOne(
                     { _id: obj.InvoiceId } ,
                     {
                         $set: {
                             ...
                             "InvoiceDate": obj.InvoiceDate,
                             "RemainingAmount": EditRemaining,
                             "InProgressAmount": EditInProgress,
                             "CurrentCreditAmount": obj.CurrentCreditAmount,
                             "UsedCurrentCreditAmount": obj.UsedCurrentCreditAmount,
                             "PaidCurrentCreditAmount": obj.PaidCurrentCreditAmount,
                             "TemporaryCreditAmount": obj.TemporaryCreditAmount,
...
exports.BuyerPayment_Approve = async function (req, res) {
   ...
   try {
       ...
       const PaidInvoiceArr = InvoiceDetails;
       ...
       if (CustomerDetails && InvoiceDetails.length) {
          ...
          const invoicePromises = InvoiceDetails.map(async (invoice) => {
                   ...
                   if (inviteResult.length !== null) {
                     ...
                     inviteResult.forEach(async resultObj => {
                         try {
                            ...
                            mSetAvailableLimit = resultObj.AvailableLimit + result_Res.PaymentAmount;
               ...
               //  mPaidAmount = invoice.PaidAmount + obj.InProgressAmount ; 
             
               if (PaidInvoiceArr.length > 1 ) {
                  PaidInvoiceArr.forEach(obj=>{
                     InvoiceDetails.map(async Obj_inv =>{
                        if (obj.InvoiceId.toString() === Obj_inv._id.toString()) {
                           // Add the InProgressAmount to mPaidAmount
                           mPaidAmount = Obj_inv.PaidAmount + obj.InProgressAmount;

                           if (Obj_inv.InProgressAmount > obj.InProgressAmount) {
                              mInProgressAmount = Obj_inv.InProgressAmount - obj.InProgressAmount
                           } else if (obj.InProgressAmount > Obj_inv.InProgressAmount) {
                              mInProgressAmount = obj.InProgressAmount - Obj_inv.InProgressAmount
                           }

                          await InvoiceManagement.InvoiceSchema.findByIdAndUpdate(obj.InvoiceId, {
                  ...
                  PaidInvoiceArr.map(async obj=>{
                  
                     mPaidAmount = invoice.PaidAmount + obj.InProgressAmount;
                     mInProgressAmount = invoice.InProgressAmount - obj.InProgressAmount;
                     await InvoiceManagement.InvoiceSchema.findByIdAndUpdate(obj.InvoiceId, {
                        PaidAmount: mPaidAmount,
                        InProgressAmount: mInProgressAmount,
                    }, { new: true });
               ...
               // const filteredInvoices = PaidInvoiceArr.filter(invoice => InvoiceArr.includes(invoice.InvoiceId));

               
               // const matchedAmounts = filteredInvoices.map(invoices => ({
               // InvoiceId: invoices.InvoiceId,
               // InProgressAmount: invoice.InProgressAmount - invoices.InProgressAmount
               // }));

               // console.log(matchedAmounts);

               // mInProgressAmount = invoice.PaidAmount + mPaidInProgressAmount;
               ...
               var x =0;
               x += invoice.InProgressAmount; 
             
               var paid = x + invoice.PaidAmount; 
               var chartUpdtae = 0;
            
              if (invoice.InvoiceAmount > paid) {
               chartUpdtae = invoice.InvoiceAmount - mPaidAmount;
              } else if (paid > invoice.InvoiceAmount){
               chartUpdtae = mPaidAmount - invoice.InvoiceAmount;
              }
              var StatusType;
              //invoice.InvoiceAmount === invoice.PaidAmount ||
              if ( invoice.InvoiceAmount === mPaidAmount ) {
                  StatusType = 'cleared';
                  invoice.PaidORUnpaid = 'Paid'
              } else {
      ...
      //  res.status(400).json({ Status: false, Message: 'An error occurred while updating invoice', Error: error });
      res.status(200).send({ Status: true, Message: "Payment Status Updated Successfully" });
   }
};
                  ...
                  if (CustomerDetails !== null && InvoiceDetails.length !== 0) {
                     PaymentManagement.PaymentSchema.updateOne(
                        ...
                        if (err_1) {
                           ...
                           var minProgressAmount = 0;
                           var mremainingAmount = 0;
                           InvoiceDetails.map(Obj => {
                             
                                 // Invoice.map(objj =>{
                                 //    minProgressAmount = Obj.InProgressAmount - objj.InProgressAmount;
                                 //    mremainingAmount = (Obj.RemainingAmount + objj.InProgressAmount);
                                 // });
                             ...
                             InvoiceManagement.InvoiceSchema.updateOne(
                              { "_id":{ $in: InvoiceArr } },
                              {
                                 $set: {
                                    // "InProgressAmount": minProgressAmount,
                                    // "RemainingAmount": mremainingAmount,
                                 }
                              }
                           ).exec();
                            
                           Invoice.map(element =>{
                              // element.RemainingAmount = element.InProgressAmount;
                              // element.InProgressAmount = Obj.InProgressAmount - element.InProgressAmount;
                           });
            ...
            Promise.all([
               PaymentManagement.PaymentSchema
                  .aggregate([
                     ...
                     //          { $project: { "InvoiceNumber": 1 ,"InvoiceDate":1,"InvoiceAmount":1} }
                     //       ],
                     //       as: 'InvoiceDetails'
                     //    }
                     // },
