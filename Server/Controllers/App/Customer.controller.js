...
exports.OwnerRegister = function (req, res) {
   ...
   if (!ReceivingData.ContactName || ReceivingData.ContactName === '') {
      ...
      Promise.all([
         ...
         InviteManagement.InviteManagementSchema.find({ Mobile: ReceivingData.Mobile, ActiveStatus: true, Invite_Status: "Pending_Approval", IfDeleted: false }, {}, {}).exec(),
      ]).then(Response => {
            ...
            CreateOwner.save(function (err, result) {
               if (err) {
                  ...
                  if (InvitedDetails !== null) {
                     InvitedDetails.forEach(invite => {
                        let updateQuery = {};
                        if (invite.Buyer === null && invite.Seller !== null) {
                           updateQuery = { Buyer: result._id };
                        } else if (invite.Buyer !== null && invite.Seller === null) {
                           updateQuery = { Seller: result._id };
                        }
...
exports.OwnerRegisterMobile = async (req, res) => {
   ...
   try {
      ...
      user.Email = Email;
      user.State = State;
      user.CustomerCategory = CustomerCategory == 'Both' ? 'BothBuyerAndSeller' : CustomerCategory;
      user.CustomerType = 'Owner';
      await user.save();
      ...
      CustomersManagement.CustomerSchema.findOne({ _id: ReceivingData.CustomerId, ActiveStatus: true, IfDeleted: false }, {}, {}).exec(function (err, result) {
         ...
         if (err) {
            ...
            if (result !== null) {
               // Previous Customer Image
               if (result.File_Name !== '') {
                  const path = 'Uploads/Customer_Profile/' + result.File_Name;
               ...
               var reportData = ReceivingData.File_Name.replace(/^data:[a-z]+\/[a-z]+;base64,/, "").trim();
               var buff = Buffer.from(reportData.replace(/\r?\n|\r/g, " "), 'base64');
               const FileName = 'Uploads/Customer_Profile/' + result._id + randomNum + '.png';
               const File_Name = result._id + randomNum + '.png';
               // console.log('FileName', FileName);
               // console.log('File_Name', File_Name);
               fsExtra.writeFileSync(FileName, buff);
               CustomersManagement.CustomerSchema.updateOne({ _id: result._id }, { File_Name: File_Name }).exec();
               res.status(200).send({ Status: true, Message: "Customer Profile Successfully updated", Response: {File_Name} });
            } else {
               res.status(400).send({ Status: false, Message: "Invalid Customer Details" });
            }
         }
      ...
      CustomersManagement.CustomerSchema.findOne({ _id: ReceivingData.CustomerId, ActiveStatus: true, IfDeleted: false }, {}, {}).exec(function (err, result) {
         if (err) {
            ...
            if (result !== null) {
               ...
               var startOfDay = moment(ReceivingData.DateOfMonth, "DD-MM-YYYY").startOf('month').toDate();
               var endOfDay = moment(startOfDay).endOf('month').toDate();
               ...
               Promise.all([
                  ...
                  var DaysInMonth = moment(startOfDay).daysInMonth();
                  ...
                  for (let Day = 1; Day <= DaysInMonth; Day++) {
                     ...
                     if (ReceivingData.CustomerCategory === 'Seller' || ReceivingData.CustomerCategory === 'Buyer') {
                        if (InvoiceDetails.length > 0) {
                           const InvoiceDetailsArr = InvoiceDetails.filter(objNew =>
                              moment(objNew.InvoiceDueDate).date() === Day);
                              
                           if (InvoiceDetailsArr.length > 0) {
                              Calender.Date = moment(InvoiceDetailsArr[0].InvoiceDueDate).format("DD-MM-YYYY");
                              var InvoiceAmount = 0;
                              var OverDueAmount = 0;
                              ...
                              InvoiceDetailsArr.map(Obj => {
                                 ...
                                 if (mInvoiceDate.isBefore(mCurrentDateDate, 'day')) {
                                    OverDueAmount += parseFloat(Obj.RemainingAmount);
                                 } else if(mInvoiceDate.isAfter(mCurrentDateDate, 'day')) {
                                    UpComingAmount += parseFloat(Obj.RemainingAmount);
                                 } else if(mInvoiceDate.isSame(mCurrentDateDate, 'day')) {
                                    DueTodayAmount += parseFloat(Obj.RemainingAmount);  
                                 }

                                 InvoiceAmount += parseFloat(Obj.RemainingAmount);
                                 ...
                                 Calender.Details.push({
                                    ...
                                    "Date": moment(Obj.InvoiceDate).format("DD-MM-YYYY"),
                                    "UniqueNumber": Obj.InvoiceNumber,
                                    "Type": "Invoice"
                                 });
                              });

                              Calender.InvoiceAmount = parseFloat(InvoiceAmount.toFixed(2));
                              Calender.OverDueAmount = parseFloat(OverDueAmount.toFixed(2));
                              Calender.DueTodayAmount = parseFloat(DueTodayAmount.toFixed(2));
                              Calender.UpComingAmount = parseFloat(UpComingAmount.toFixed(2));
                           }
                        }
                     }
                  ...
                  ErrorHandling.ErrorLogCreation(req, 'CustomerProfileUpload Getting Error', 'Common.controller -> CustomerProfileUpload', JSON.stringify(Error));
                  res.status(417).send({ Status: false, Message: "Some errors predicted, We are unable to get CustomerProfileUpload", Error: Error });
               });
            } else {
               res.status(400).send({ Status: false, Message: "Invalid Customer Details" });
            }
...
exports.CustomerProfileDelete = async (req, res) => {
   const { CustomerId } = req.body;
 
   if (!CustomerId || CustomerId === '') {
     return res.status(400).send({ Status: false, Message: "Customer details can not be empty" });
   }
 
   try {
     const customer = await CustomersManagement.CustomerSchema.findOne({
     ...
     if (!customer) {
       return res.status(400).send({ Status: false, Message: "Invalid Customer Details" });
     }

     if (customer.File_Name!== '') {
       const filePath = `Uploads/Customer_Profile/${customer.File_Name}`;
       if (fs.existsSync(filePath)) {
         fs.unlinkSync(filePath);
         await CustomersManagement.CustomerSchema.updateOne({ _id: customer._id }, { File_Name: '' });
         return res.status(200).send({ Status: true, Message: "Customer Profile Successfully deleted" });
       } else {
         return res.status(400).send({ Status: false, Message: "File does not exist at path:", filePath });
       }
     } else {
       return res.status(400).send({ Status: false, Message: "No file found for deletion" });
     }
   } catch (err) {
     ErrorHandling.ErrorLogCreation(req, 'CustomerProfileDelete Getting Error', 'Common.controller -> CustomerProfileDelete', JSON.stringify(err));
     return res.status(417).send({ Status: false, Message: "Some errors predicted, We are unable to get CustomerProfileDelete", Error: err });
   }
 };
