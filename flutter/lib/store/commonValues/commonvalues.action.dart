...
ThunkAction<AppState> getNotificationsList = (Store<AppState> store) async {
  ...
  try{
    ...
    final response = await http.post(
      Uri.parse('${AppConfig.rootUrl}/APP_API/CommonManagement/All_Notifications_List'),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
        'Authorization': 'Bearer ${store.state.commonValuesState.firebaseToken}',
      },
    ...
    if (notificationsList['Success'] == true) {
      store.dispatch(UpdateNotificationsListAction(notificationsList['Response']));
...
ThunkAction<AppState> markNotificationRead (Map data) {
  return (Store<AppState> store) async {
    try {
      final response = await http.post(
        Uri.parse('${AppConfig.rootUrl}/APP_API/CommonManagement/Notification_Viewed_Update'),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
          'Authorization': 'Bearer ${store.state.commonValuesState.firebaseToken}',
        },
