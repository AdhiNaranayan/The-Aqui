  ...
  Widget build(BuildContext context) {
    ...
    return StoreConnector<AppState, Store<AppState>>(
      ...
      builder: (BuildContext context, store) {
        ...
        final notificationsList = store.state.commonValuesState.notificationsList;
        final notificationsListLoaded = store.state.commonValuesState.notificationsListLoaded;
    ...
    return Scaffold(
     key: scaffoldKey,
          appBar: PreferredSize(
            ...
            child: WidgetHelper.getAppBar(context, 'Notifications', openDrawer, customerCurrentScreen == 'Seller' ? Colors.orange : Colors.deepPurple.shade900, onAddButtonClick),
          ),
          ...
          ? Center(
            child: CircularProgressIndicator(
              valueColor: AlwaysStoppedAnimation<Color>(customerCurrentScreen == 'Seller' ? Colors.orange : Colors.deepPurple.shade900),
            ),
          ...
          ListView.builder(
                ...
                itemBuilder: (BuildContext context, int index) {
                  return Container(
                    margin: EdgeInsets.only(left: screenWidth * 0.05, right: screenWidth * 0.05, top: screenWidth * 0.05),
                    padding: EdgeInsets.all(screenWidth * 0.05),
                    decoration: BoxDecoration(
                      color: notificationsList[index]['Message_Viewed'] == false ? Colors.white : Colors.grey.shade100,
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          notificationsList[index]['Notification_Type'],
                          style: TextStyle(
                            color: notificationsList[index]['Message_Viewed'] == false ? Colors.black : Colors.grey.shade700,
                            fontSize: AppConfig.size(context, 16),
                            fontWeight: notificationsList[index]['Message_Viewed'] == false ? FontWeight.bold : FontWeight.normal,
                          ),
                        ),
                        ...
                        SizedBox(height: screenWidth * 0.02),
                        Text(
                          DateFormat('DD MMM yyyy').format(DateTime.parse(notificationsList[index]['createdAt'])),
                          style: TextStyle(
                            color: notificationsList[index]['Message_Viewed'] == false ? Colors.black : Colors.grey.shade700,
                            fontSize: AppConfig.size(context, 14),
                          ),
                        ...
                        SizedBox(height: screenWidth * 0.02),
                        notificationsList[index]['Message_Viewed'] == false
                        ? InkWell(
                          onTap: () {
                            store.dispatch(markNotificationRead({'NotificationId': notificationsList[index]['_id']}));
                          },
                          child: Container(
                            padding: EdgeInsets.all(screenWidth * 0.02),
                            decoration: BoxDecoration(
                              color: customerCurrentScreen == 'Seller' ? Colors.orange : Colors.deepPurple.shade900,
