...
class WidgetHelper {
  static Widget getAppBar(BuildContext context, String title, Function() onMenuClick, Color backgroundColor, Function onAddClick) {
    return Scaffold(
      appBar: AppBar(
        ...
        [
          title == 'Dashboard' ?
          WidgetHelper.getNotificationIcon(context) :
          title == 'My Business' ?
          WidgetHelper.addBusinessIcon(context, () {
            ...
            showDialog(
              ...
              builder: (BuildContext context) {
                return WidgetHelper.addBusinessModalForm(context, 'Add Business');
              },
            );
          }) : 
          ...
          :
          title == 'Seller\'s List' ?
          IconButton(onPressed: onAddClick(), icon: const Icon(Icons.add_circle_outline, color: Colors.white)) : 
          title == 'Invite History' ?
          WidgetHelper.inviteBuyerIcon(context, () {
            // show invite buyer modal form
            onAddClick();
          }) :
          (title == 'Invoices' || title == 'Users' || title == 'Support') ?
  ...
  static getNotificationIcon(BuildContext context, {int badgeCount = 2}) {
            ...
            Positioned(
              ...
              child: Container(
                ...
                child: Text(
                  ...
                  style: TextStyle(
                    color: Colors.white,
                    fontSize:  AppConfig.isPortrait(context) ? AppConfig.size(context, 10) : AppConfig.size(context, 12),
                  ),
  ...
  static createInvoiceIcon(BuildContext context, Function() onCreateInvoiceClick) {
    return IconButton(
      icon: const Icon(
        Icons.add_circle_outline,
        color: Colors.white,
  ...
  static Widget leftNavigationBar(BuildContext context, screenIndex, contactName, contactNumber, Color backgroundColor) {

    final customerSelectedScreen = StoreProvider.of<AppState>(context).state.dashboardState.customerCurrentScreen;
    final mobileNumber = StoreProvider.of<AppState>(context).state.authState.customerData['Mobile'];
    ...
    void handleLogoutConfirm(int index) {
      ...
      if (index == 0) {
        ...
        StoreProvider.of<AppState>(context).dispatch(UpdateSelectedBottomNavIndexAction(0));
        // show add business modal form
        Future.delayed(Duration.zero, () {
              Navigator.push(
                context,
      ...
      if (index == 1) {
        StoreProvider.of<AppState>(context).dispatch(UpdateSelectedBottomNavIndexAction(1));
        // show add business modal form
        Future.delayed(Duration.zero, () {
              Navigator.push(
                context,
      ...
      if (index == 3) {
        StoreProvider.of<AppState>(context).dispatch(UpdateSelectedBottomNavIndexAction(3));
        // show add business modal form
        Future.delayed(Duration.zero, () {
              Navigator.push(
                context,
      ...
      if (index == 4) {
        StoreProvider.of<AppState>(context).dispatch(UpdateSelectedBottomNavIndexAction(4));
        // show add business modal form
        Future.delayed(Duration.zero, () {
              Navigator.push(
                context,
      ...
      if (index == 5) {
        StoreProvider.of<AppState>(context).dispatch(UpdateSelectedBottomNavIndexAction(2));
        // show add business modal form
        Future.delayed(Duration.zero, () {
              Navigator.push(
                context,
        ...
        children: <Widget>[
              ...
              children: [
                const CircleAvatar(
                  ...
                  backgroundImage: // if image available
                      AssetImage('assets/images/profile-empty.png'),
                ),
                // show name
                 Text(
                  contactName ?? '',
          ...
          ...destinations.map(
            (MenuItems destination) {
              customerSelectedScreen == 'Buyer' && destination.label == 'Buyers' ? destination = destinations2[2] : destination = destination;
  ...
  static Widget addBusinessModalForm (BuildContext context, String title) {
    ...
    Map<String, dynamic> industryList1 = StoreProvider.of<AppState>(context).state.businessState.industryList;
    ...
    TextEditingController businessCreditLimitController = TextEditingController();
    final customerCurrentScreen = StoreProvider.of<AppState>(context).state.dashboardState.customerCurrentScreen;
    ...
    void saveForm() {
      ...
      StoreProvider.of<AppState>(context).dispatch(addBusiness(
        ...
        selectedIndustry ?? '',
        businessCreditLimitController.text != '' ? double.parse(businessCreditLimitController.text) : 0.0,
      ));
    }
        ...
        child: SingleChildScrollView(
          child: ListBody(
            children: <Widget>[
              ...
              TextFormField(
                decoration: const InputDecoration(labelText: 'Business Credit Limit'),
...
static Widget editBusinessModalForm (BuildContext context, String businessId, String businessName, String branchName, double businessCreditLimit, Map<String, dynamic> industryId) {
    final GlobalKey<FormState> formKey = GlobalKey<FormState>();
    Map<String, dynamic> industryList1 = StoreProvider.of<AppState>(context).state.businessState.industryList;
    String? selectedIndustry;
    // store industry list as array from industryList['Response']
...
const List<MenuItems> destinations = <MenuItems>[
  MenuItems(
      'My Business', Icon(Icons.business_center_outlined), Icon(Icons.business_center)),
  MenuItems(
  ...
  MenuItems(
   ...
   MenuItems(
      'Invoices', Icon(Icons.document_scanner_outlined), Icon(Icons.document_scanner)),
  MenuItems(
      'Payments', Icon(Icons.payment_outlined), Icon(Icons.payment)),
  MenuItems(
    'My Profile', Icon(Icons.person_outline), Icon(Icons.person)),
  MenuItems(
    'User Management', Icon(Icons.people_outline), Icon(Icons.people)),
  MenuItems(
...
const List<MenuItems> destinations2 = <MenuItems>[
  ...
  MenuItems(
      'Invite List', Icon(Icons.groups_2_outlined), Icon(Icons.groups_sharp)),
  MenuItems(
      'Sellers', Icon(Icons.store_outlined), Icon(Icons.store)),
   MenuItems(
      'Invoices', Icon(Icons.document_scanner_outlined), Icon(Icons.document_scanner)),
  MenuItems(
      'Payments', Icon(Icons.payment_outlined), Icon(Icons.payment)),
