...
@Component({
  selector: 'app-modal-customer-edit',
  templateUrl: './modal-customer-edit.component.html',
  styleUrls: ['./modal-customer-edit.component.css']
})
export class ModalCustomerEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.Type === 'Edit') {
      this.CustomerForm = new FormGroup({
        Current_Mobile : new FormControl(this.UserDetails.Mobile),
        Mobile : new FormControl(this.UserDetails.Mobile),
        ContactName: new FormControl(this.UserDetails.ContactName, Validators.required),
  ...
  Update() {
    if (this.CustomerForm.valid && !this.Uploading) {
      ...
      this.CustomerService.Customer_Update(Info).subscribe(response => {
  ...
  GetFormControlErrorMessage(KeyName: any) {
    const FControl = this.CustomerForm.get(KeyName) as FormControl;
  ...
  CommonInputReset(control: any, value: any) {
    this.CustomerForm.controls[control].setValue(value);
    this.CustomerForm.controls[control].clearValidators();
    this.CustomerForm.controls[control].setErrors(null);
    this.CustomerForm.controls[control].markAsPristine();
    this.CustomerForm.controls[control].markAsUntouched();
  ...
  ClearInput(event: KeyboardEvent): boolean {
     ...
     this.CustomerForm.controls[FControlName].setValue(null);
     return false;
  }
